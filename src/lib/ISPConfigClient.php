    ...
    private function initializeSoapClient(): void
    {
        try {
            ...
            $streamContextOptions = [
                ...
                'socket' => [
                    'bindto' => '0:0' // Force IPv4 by binding to IPv4 address
                ]
            ];
            
            // Use getaddr info to resolve IPv4 only
            $host = parse_url($this->config['soap_location'], PHP_URL_HOST);
            if ($host && filter_var($host, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) === false) {
                ...
                if ($ipv4 !== $host && filter_var($ipv4, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
                    ...
                    $soapOptions['location'] = str_replace($host, $ipv4, $this->config['soap_location']);
                }
            }
            
            $soapOptions['stream_context'] = stream_context_create($streamContextOptions);
