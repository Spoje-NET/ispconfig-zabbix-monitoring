...
class ZabbixHelper
{
    ...
    public function formatDiscovery(array $items, array $macroMap): array
    {
        ...
        foreach ($items as $item) {
            ...
            foreach ($macroMap as $sourceKey => $macroName) {
                $value = self::getNestedValue($item, $sourceKey);
                $discoveryItem[$macroName] = self::sanitizeValue($value);
            }
    ...
    public function formatItemValue($value, string $type = 'string'): string
    {
        ...
        switch ($type) {
            ...
            case 'boolean':
                return self::parseBooleanValue($value) ? '1' : '0';
            case 'timestamp':
                return self::formatTimestamp($value);
            case 'bytes':
                return (string) self::parseBytes($value);
    ...
    public function createItemKey(string $prefix, string $metric, array $params = []): string
    {
        ...
        if (!empty($params)) {
            $paramString = implode(',', array_map(static function ($param) {
                return self::escapeKeyParam($param);
            }, $params));
            $key .= "[{$paramString}]";
        }
     ...
     * @return mixed
     */
    private static function getNestedValue(array $array, string $key, $default = '')
    {
     ...
     * @param mixed $value
     */
    private static function parseBooleanValue($value): bool
    {
        if (\is_bool($value)) {
            return $value;
        }
     ...
     * @return int Bytes
     */
    private static function parseBytes($value): int
    {
        if (is_numeric($value)) {
            return (int) $value;
        }
     ...
     */
    private static function escapeKeyParam(string $param): string
    {
